...
function cookie(config: ServerConfig, sub: string): string {
  return (
    'wn_session=' +
    seal({ exp: Math.floor(Date.now() / 1000) + 300, sub, email: `${sub}@x.test`, name: sub }, config.sessionSecrets)
  )
}
...
describe('role resolver', () => {
  it('provisions a rowless session and publishes its role on req.user', async () => {
    ...
    const app = await buildApp({ config, pages: createMemoryPagesRepository(), users, relyingParty: null })
    const res = await app.inject({ method: 'GET', url: '/api/me', headers: { cookie: cookie(config, 'fresh') } })
  ...
  it('grants DEFAULT_ROLE to later accounts', async () => {
    ...
    const app = await buildApp({ config, pages: createMemoryPagesRepository(), users, relyingParty: null })
    const res = await app.inject({ method: 'GET', url: '/api/me', headers: { cookie: cookie(config, 'newcomer') } })
  ...
  it('bootstrap-list subs get admin even with a populated table', async () => {
    ...
    const users = usersFixture({ bootstrapAdminSubs: ['chosen-one'], defaultRole: 'editor', users: { boss: 'admin' } })
    const app = await buildApp({ config, pages: createMemoryPagesRepository(), users, relyingParty: null })
    const res = await app.inject({ method: 'GET', url: '/api/me', headers: { cookie: cookie(config, 'chosen-one') } })
  ...
  it('role changes take effect on the very next request (no cookie churn)', async () => {
    ...
    expect(
      (await app.inject({ method: 'GET', url: '/api/me', headers: { cookie: ck } })).json().user.role,
    ).toBe('admin')
  ...
  it('resolves from the users row, not cookie claims — and leaves the row untouched', async () => {
    ...
    const app = await buildApp({ config, pages: createMemoryPagesRepository(), users, relyingParty: null })
    const res = await app.inject({ method: 'GET', url: '/api/me', headers: { cookie: cookie(config, 'alice') } })
