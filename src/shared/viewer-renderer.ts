// ─── Shared markdown-it viewer renderer ──────────────────────────────────────
// The ONE markdown engine for "reader parity": the server read path renders
// pages with it, and any future browser-side preview uses the same code for
// identical output. markdown-it is pure JS (string → tokens → HTML), so this
// module stays environment-agnostic and bundleable into the client.
//
// CommonMark + default extras (tables, ~~strike~~) with html:false so
// author-written HTML is escaped, our URL policy for hrefs, a [[wiki-link]]
...
import { wikiTargetToSlug } from './slug'
