...
/**
 * Minimal token definition that matches any word characters at the start
 * of remaining text.  Used to verify the tokenize → render pipeline runs.
 */
...
/**
 * A no-op plugin that renders text tokens as-is but wraps them in a <span>
 * so we can detect that the rendering pipeline ran.
 */
...
describe("createEditorRender: render()", () => {
  ...
  // Test 6: navigateFn is passed to state.toContext during rendering
  it("passes navigateFn to state.toContext during render", () => {
    const navigateFn = vi.fn()

    // A plugin that calls context.navigate during rendering
    const navPlugin: Plugin = {
      name: "nav-test",
      tokens: [{ type: "navigate", pattern: /^trigger/ }],
    ...
    const render = createEditorRender(dom, [navPlugin], state, { navigateFn })

    dom.editorDiv.textContent = "trigger"
    render.render()

    expect(navigateFn).toHaveBeenCalledWith("linked-page")
  })
})
...
describe("createEditorRender: syncUrlToTrail()", () => {
  ...
  it("updates URL via history.replaceState", () => {
    ...
    expect(url).toContain("path=home%2Fabout")
  })
